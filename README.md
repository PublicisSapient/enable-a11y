...
- Lynx: A text-only browser used for testing for how a website will work without graphics turned on.
  ...
  - Note(for windows users):
    - it would be recommended to visit the [Cygwin](https://cygwin.com/) and choose the Install or update the link.
    - it would be recommended to install [Git Bash](https://git-scm.com/downloads) and then inside `.bash_profile` add `export PATH="$PATH:/c/cygwin64/bin/"` - make sure cygwin folder is same as mentioned or else modify the `/c/cygwin64/bin/` in the `.bach_profile` accordigly
- PHP (version > 8.1)
  - Install options:
