        ...
        `);
            ...
            domInfo = await page.evaluate(
                (i, numSlidesInCarousel) => {
                    ...
                    let currentSlideRect =
                        $currentSlide.getBoundingClientRect();
                    let containerRect = $container.getBoundingClientRect();
                    let previousSlideRect =
                        $previousSlide.getBoundingClientRect();
                    ...
                    const isCurrentSlideInView =
                        containerRect.x <= currentSlideRect.x &&
                        currentSlideRect.right <= containerRect.right;

                    const isPrevSlideOutOfView =
                        containerRect.x > previousSlideRect.x ||
                        previousSlideRect.right > containerRect.right;

                    const isNextSlideOutOfView =
                        containerRect.x > nextSlideRect.x ||
                        nextSlideRect.right > containerRect.right;
    ...
    it('Test carousel type #2 (list of content)', async () => {
        ...
        domInfo = await page.evaluate(() => {
            ...
            const ariaDescribedby =
                $prevButton.getAttribute('aria-describedby');
            const $instructions = ariaDescribedby
                ? document.getElementById(ariaDescribedby)
                : null;
            const hasInstructions =
                ariaDescribedby &&
                $instructions &&
                $instructions.innerHTML.trim() !== '';
        ...
        for (let i = 1; i < numSlidesInCarousel; i++) {
            ...
            domInfo = await page.evaluate((i) => {
                ...
                return {
                    isFocusInVisiblePanel:
                        activeElement === $currentSlide ||
                        activeElement.closest('.enable-carousel__slide') ===
                            $currentSlide,
                    $currentSlide: $currentSlide.outerHTML,
                    activeElement: activeElement.outerHTML,
                };
