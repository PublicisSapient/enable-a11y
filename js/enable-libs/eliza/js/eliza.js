...
const eliza = new function () {
	...
	this.startChat = () => {
		...
		this.chatHistory.push({ isEliza: true, content: initialMessage });
		elizaActions.displayChat(this);
	 ...
	 */
	this.sendElizaNewMessage = (newMessage) => {

		//Add to UI
		this.chatHistory.push({ isEliza: false, content: newMessage });
		elizaActions.displayChat(this);
		elizaActions.clearSendTextbox();
		...
		setTimeout(() => {
			...
			this.chatHistory.push({ isEliza: true, content: response });
			elizaActions.displayChat(this);
