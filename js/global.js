...
function initEnable() {
  ...
  const breakpointWidth = window.getComputedStyle(document.querySelector('.enable-flyout')).getPropertyValue('--enable-flyout__desktop-min');
  ...
  if (location.href.indexOf('index.php') === -1) {
    document.querySelectorAll('h1, h2, h3, h4, h5, h6, [role="heading"]').forEach((el) => {
      ...
      if (el.closest('.enable-example') === null && el.closest('.no-permalink-headings') === null && el.closest('.cookie-banner') === null) {
    ...
    document.addEventListener(
      ...
      (event) => {
        for (var i = 0; i < properties.length; i++) {
          ...
          console.log(`${eventName} fired. ${property}:`, event.detail && event.detail[property] ? event.detail[property]() : '', 'target:', event.target);
        }
      },
      true,
    );
