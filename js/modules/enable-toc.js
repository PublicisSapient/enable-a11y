...
const tableOfContents = new (function () {
    ...
    const commonSelectors = () => {
        return {
            ...
            toggleButtonSelector: document.querySelector(
        ...
        document
            ...
            .forEach((el) => {
                ...
                const headingLevel = Number(
                    el.nodeName?.toLowerCase()?.split('h')?.[1] || 0,
                );
                ...
                if (
                    (selectorToSkipHeadingsWithin &&
                        el.closest(selectorToSkipHeadingsWithin) !== null) ||
                    (ignoreHeadersDeeperThan &&
                        headingLevel > ignoreHeadersDeeperThan)
                ) {
                    ...
                    if (
                        ...
                        expandButton.setAttribute(
                            'aria-label',
                            `Links for the content under the heading`,
                        );
                        expandButton.setAttribute(
                            'aria-controls',
                            `enable-drawer${elementCount}__content`,
                        );
                        expandButton.innerHTML =
                            '<img src="/images/plus.svg" alt="" />';
                        expandButton.addEventListener(
                            'click',
                            this.expandButtonClick,
                        );
                ...
                tocItem.classList.add(
                    `enable-toc__item-${el.tagName.toLowerCase()}`,
                );
    ...
    this.closeToggleTOCOnEvent = (event) => {
        ...
        if (
            ...
            (event.target.parentNode &&
                !toggleButtonSelector?.contains(event.target) &&
                !toggleTOCSelector?.contains(event.target))
        ) {
    ...
    this.toggleTOC = () => {
        ...
        const isExpanded =
            toggleButtonSelector?.getAttribute('aria-expanded') === 'true';
        if (isExpanded) {
            this.closeToggleTOC();
        } else {
            this.openToggleTOC();
    ...
    this.appendAsSidebar = () => {
        ...
        hideSidebarButton.setAttribute(
            'aria-label',
    ...
    this.appendAsToggleButton = () => {
        ...
        moveToSidebarButton.setAttribute(
            'aria-label',
        ...
        const expandedContent = clonedToc.querySelectorAll(
        ...
        const expandButtons = clonedToc.querySelectorAll(
        ...
        // Add event listeners to TOC links to close the TOC on click
        const tocLinks = clonedToc.querySelectorAll('a.enable-toc__link');
    ...
    this.moveToSidebar = () => {
        ...
        document
            ...
            .classList.add('enable-toc-as-sidebar');
    ...
    this.moveToToggleButton = () => {
        ...
        document
            .getElementsByTagName('body')[0]
            .classList.remove('enable-toc-as-sidebar');

        // Update the toggle button tooltip
        const { toggleButtonSelector } = commonSelectors();
        toggleButtonSelector?.setAttribute(
            'data-tooltip',
            'The Table of Contents has moved here. Click to open or close.',
        );
    ...
    this.init = ({
        ...
        if (
            sidebarCookieValue === 'true' ||
            (!sidebarCookieValue && showAsSidebarDefault)
        ) {
            document
                .getElementsByTagName('body')[0]
                .classList.add('enable-toc-as-sidebar');
