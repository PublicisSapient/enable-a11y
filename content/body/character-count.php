<p>
  ...
  of an accessible character-counter. Both variations do the same basic things:
</p>

<ol>
  <li>When a keyboard user focuses into a textarea with a character-counter, screen readers will announce how many
    characters the user has typed in and how many characters they are all owed to type.</li>
...
<ul>
  ...
  <li>The top variation will announce to screen reader users this information if the types a Space (i.e. when the user
    finishes typing a word). This announcement is delayed by half a second so that it is not announced if the user is
    typing quickly.</li>
  <li>The bottom variation will announce to screen reader users the character count information if the user presses
    Escape.</li>
</ul>

<p>
  Which do you like better? Or do you think these two variations should be combined? Should the developer be able to
  configure which of these two features will be activated? Would you add anything? Or should we do something else?
  Please feel free to contact me via either <a href="https://mastodon.social/@zoltandulac">my Mastodon account</a> or <a
    href="https://twitter.com/zoltandulac">my Twitter account</a> and let me know. <strong>Whatever we end up
    implementing will be released as an open source JavaScript library via github and NPM, so the community that helps
    out here will definitely benefit!</strong>
</p>

<h2>First Variation: Count Announced After Typing A Word.</h2>

<p>I think that waiting for the user to finish a word and press space before the character count is announced is better
  than having the character count announced after every keystroke (which I think would be annoying).
</p>
...
<p>I am not sure this is a good idea (since Escape is used to leave forms mode in NVDA), but I put this example here to
  start a conversation about any alternatives to this that may make sense</p>
