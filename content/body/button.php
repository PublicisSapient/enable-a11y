<aside class="notes">
    <h2>Notes:</h2>
    <ul>
        <li>It is always better to use a real HTML <code>button</code> tag than a <code>div</code> or <code>a</code> tag with a <code>role="button"</code>. This is especially noticeable when using a <code>label</code> with the <code>button</code>, which reads differently in some screen readers then when using an <code>aria-describedby</code> to give it supplementary information. </li>
        <li> Links can look like buttons, and buttons can look like links. Adam Silver has written a great article, <a href="https://medium.com/simple-human/but-sometimes-links-look-like-buttons-and-buttons-look-like-links-9b371c57b3d2">But sometimes links look like buttons (and buttons look like links)</a> about this topic. The main difference, from a UX point of view, is that links go to another web page, while buttons cause an action on a page (or submit a form). Keep this in mind when you are coding your CTAs. We should be marking up these controls correctly so screen reader users know in advance what will happen when they interact with them. </li>
    </ul>
</aside>
<p> <strong>A button is an interactive element that triggers an action on a webpage or submits a form.</strong> The <code>&lt;button&gt;</code> tag has been available for a long time and developers should utilize it for this specific purpose. </p>
<p> This may seem obvious to many developers. Unfortunately, there is a lot of code out there that uses <code>&lt;a&gt;</code> tags to do the work of a <code>&lt;button&gt;</code>. This is problematic from an accessibility standpoint because those relying on screen readers may mistakenly assume that clicking it will redirect them to another page. When I review code like this, it makes me sad. If you do this in a new project, think about how sad I will be, and change your ways. </p>
<p> This page covers how to make buttons in three ways. Review all scenarios and choose the one that fits your requirements the best, but make sure to take into account the warnings provided before making your final decision. </p>
...
<div class="enable-example">
    <p>The following is a <code>&lt;button&gt;</code> tag with a <code>&lt;label&gt;</code> tag describing what it is for. </p>
    <div id="example1" class="button-container">
        ...
        <button id="html-button"> Submit </button>
    </div>
</div>
...
<div id="example3" class="button-container enable-example">
    ...
    <a class="aria-button" aria-describedby="link-button-label" href="#" role="button"> Submit </a>
</div>
...
<div id="example2" class="button-container enable-example">
    ...
    <div class="aria-button" aria-describedby="div-button-label" role="button" tabindex="0"> Submit </div>
</div>
...
<ol>
    <li>Use the <code>disabled</code> attribute. This prevents click events from being fired. It also removes the button from the keyboard tabbing order, which makes it harder for screen reader users to know that the disabled button even exists. </li>
...
<div class="enable-example">
    ...
    <div id="html-disabled" class="button-container">
        <label for="html-disabled-button">If you are sure you want to give Facebook your data, push this:</label>
        <button disabled id="html-disabled-button"> Submit </button>
    </div>
    <p> The following button is disabled with aria-disabled="true"</p>
    <div id="aria-disabled" class="button-container">
        <label for="aria-disabled-button">If you are sure you want to give Facebook your data, push this:</label>
        <button aria-disabled="true" id="aria-disabled-button"> Submit </button>
    </div>
    <p> The following button is disabled with aria-disabled="true" and JavaScript</p>
    <div id="aria-js-disabled" class="button-container">
        <label for="aria-js-disabled-button">If you are sure you want to give Facebook your data, push this:</label>
        <button aria-disabled="true" id="aria-js-disabled-button"> Submit </button>
    </div>
</div>
