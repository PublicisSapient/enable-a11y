...
<div id="example2" class="enable-example">
    <div class="enable-checkbox">
        <label class="form-control"> <input type="checkbox" checked id="checkbox_1"> I agree to sell my soul to Zoltan </label>
    </div>
</div>
...
<p> If you come across a <code>&lt;div&gt;</code> in existing code that is marked up like a checkbox, you can fix it this way. It is preferable to use the HTML5 version instead if you can implement it quickly. </p>
...
<script type="application/json" id="example-role-checkbox-props">
    {
        ...
        "steps": [{
            ...
            {
                "label": "Add custom styles",
                "highlight": "%CSS%checkbox-css~ [role=\"checkbox\"] ||| outline[^;]*;  ; [role=\"checkbox\"][aria-checked=\"true\"]::after",
...
<?php includeStats(["isForNewBuilds" => true]); ?>
<p> If you have a group of checkboxes, this is the proper way to style them. Instead of fieldsets, you could use <code>&lt;role="group"&gt;</code>, which is described in the <a href="form.php#aria">ARIA form role example</a>. </p>
<div id="group-example" class="enable-example">
    <form id="group-example__form">
        <fieldset>
            <legend> The following people will have my soul when I die: </legend>
            ...
            <div class="checkbox-container enable-checkbox">
                <label class="form-control"> <input id="html-checkbox-multi1" type="checkbox" aria-invalid="true" aria-describedby="html-checkbox__error"> Zoltan</label>
            </div>
            <div class="checkbox-container enable-checkbox">
                <label class="form-control"> <input id="html-checkbox-multi2" type="checkbox" aria-invalid="true" aria-describedby="html-checkbox__error"> Noel</label>
            </div>
            <div class="checkbox-container enable-checkbox">
                <label class="form-control"> <input id="html-checkbox-multi3" type="checkbox" aria-invalid="true" aria-describedby="html-checkbox__error"> Alison</label>
            </div>
            <div class="checkbox-container enable-checkbox">
                <label class="form-control"> <input id="html-checkbox-multi4" type="checkbox" aria-invalid="true" aria-describedby="html-checkbox__error"> That guy who looks like Gandalf who smokes in the alleyway at the office</label>
            </div>
...
<p> We usually think of checkboxes being either <strong>checked</strong> or <strong>unchecked</strong>. There is a third possible state: <strong>indeterminate</strong>. The most common use case for this, as outlined in <a href="https://css-tricks.com/indeterminate-checkboxes/">the CSS Tricks article about Indeterminate Checkboxes</a>, is for nested checkboxes. Consider you have a group of related attributes, like toppings in an ice-cream cone, that you can select with a group of checkboxes. It would be great to be able to have a "select all" checkbox that can choose all of them &mdash; because who doesn't want all the things on their ice cream cone?!?! The problem is, what is this "select all" checkbox set to when some of the ingredients are checked but not all of them? While I have seen select all checkboxes not checked in this case, it can be argued that we give the indeterminate state for the checkbox. </p>
<p><strong>Note that this indeterminate state can be set via JavaScript.</strong> There is no <code>indeterminate</code> HTML attribute. It is set like this:</p>
<figure class="wide">
    <?php includeShowcode("indeterminate-js", "", "", "", false); ?>
    <figcaption>How to use the indeterminate property in JavaScript</figcaption>
</figure>
<template id="indeterminate-js" data-showcode-is-js="true"> // Let's assume <input id="my-checkbox" type="checkbox"> // is in the page. const checkboxEl = document.getElementById("my-checkbox"); // Setting the indeterminate attribute checkboxEl.indeterminate = true; // We should also set the checked value to false. // This is for progressive enhancement. checkboxEl.checked = false; </template>
<p>Note the last bit in the code above where we set the <code>.checked</code> property to <code>false</code>. This is done for progressive enhancement. While <a href="https://developer.mozilla.org/en-US/docs/Web/CSS/:indeterminate#browser_compatibility">MDN reports that indeterminate is supported in all major browsers</a>, <strong>we should still consider that not all browser/screen reader pairs (e.g. Firefox/NVDA, Firefox/Voiceover at the time of this writing) will announce the indeterminate state</strong>. For this reason, we should set the checked attribute to something that makes sense for screen reader users who use those browser/screen reader pairs. Setting the checkbox to unchecked does make the most sense in this "Select All" scenario. </p>
...
<div id="indeterminate-example" class="enable-example">
    <form  onsubmit="alert('Your desert choice has been submitted.'); return false;">
        <fieldset>
            ...
            <div>
                <div class="checkbox-container enable-checkbox">
                    <label for="select-all" class="form-control"> <input type="checkbox" id="select-all" name="select-all" aria-describedby="select-all__desc" data-select-all-for="option-1 option-2 option-3"> Select All</label>
                    <span id="select-all__desc" class="sr-only">Checking this will automatically check the ingredient checkboxes below.</span>
                </div>
                <ul>
                    <li class="checkbox-container enable-checkbox">
                        <label for="option-1" class="form-control"> <input type="checkbox" id="option-1" name="ingredient" value="option-1"> Chocolate Syrup</label>
                    </li>
                    <li class="checkbox-container enable-checkbox">
                        <label for="option-2" class="form-control"> <input type="checkbox" id="option-2" name="ingredient" value="option-2"> Strawberry Sauce</label>
                    </li>
                    <li class="checkbox-container enable-checkbox">
                        <label for="option-3" class="form-control"> <input type="checkbox" id="option-3" name="ingredient" value="option-3"> Bran Flakes</label>
                    </li>
                </ul>
            </div>
            <button type="submit">Submit</button>
...
<p>This example uses a library we developed to set up the hierarchical structure for the select all button to work. Below are the developer notes on how the library does it. If you are interested in using the library, please <a href="#npm-instructions">read the instructions on how to use the library in your projects</a>.</p>
...
<p> Just like two-state checkboxes, we can use ARIA to create faux checkboxes. At the time of this writing, there is one advantage of doing so: Firefox using Voiceover and NVDA will report an indeterminate checkbox's state as mixed using ARIA. </p>
<figure class="wide">
    <?php includeShowcode("aria-indeterminate-js", "", "", "", false); ?>
    <figcaption>How to use the indeterminate property in JavaScript</figcaption>
</figure>
<template id="aria-indeterminate-js" data-showcode-is-js="true"> // Let's assume <div role="checkbox" id="my-aria-checkbox" type="checkbox">...</div> // is in the page. const checkboxEl = document.getElementById("my-aria-checkbox"); // Setting the aria-checked attribute to mixed. checkboxEl.setAttribute('aria-checked', 'mixed'); </template>
            ...
            <div>
                <div class="checkbox-container enable-checkbox">
                    <div role="checkbox" aria-checked="false" tabindex="0" aria-labelledby="aria-select-all__label"
                        ...
                        aria-describedby="aria-select-all__desc">
                    </div>
                    <label id="aria-select-all__label" class="form-control">Select All </label>
                    <span id="aria-select-all__desc" class="sr-only">Checking this will automatically check the ingredient checkboxes below.</span>
                </div>
                <ul>
                    <li class="checkbox-container enable-checkbox">
                        <div id="aria-option-1" role="checkbox" aria-checked="false" tabindex="0" aria-labelledby="aria-option-1__label">
                        </div>
                        <label id="aria-option-1__label" class="form-control">Chocolate Syrup </label>
                    </li>
                    <li class="checkbox-container enable-checkbox">
                        <div id="aria-option-2" role="checkbox" aria-checked="false" tabindex="0" aria-labelledby="aria-option-2__label">
                        </div>
                        <label id="aria-option-2__label" class="form-control">Strawberry Sauce </label>
                    </li>
                    <li class="checkbox-container enable-checkbox">
                        <div id="aria-option-3" role="checkbox" aria-checked="false" tabindex="0" aria-labelledby="aria-option-3__label">
                        </div>
                        <label id="aria-option-3__label" class="form-control">Bran Flakes </label>
                    </li>
                </ul>
            </div>
            <button type="submit">Submit</button>
...
<p>This example uses the same library we used in the native HTML5 example to set up the hierarchical structure for the select all button to work. As you compare the developer notes below to that of the HTML5 example, you will see the way to implement is similar. Please <a href="#npm-instructions">read the instructions on how to use the library in your projects</a>.</p>
