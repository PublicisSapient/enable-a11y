...
<p> Radio buttons are the easiest way to get users to choose one of a <em>small</em> set of choices. Many developers forget that a radio button has <strong>two</strong> labels: one unique to each radio button, and one for the entire group. </p>
<p> The examples below are from <a href="https://www.w3.org/WAI/GL/wiki/Using_grouping_roles_to_identify_related_form_controls">Using Grouping Roles to Identify Related Form Controls</a> from the <a href="https://www.w3.org/">W3C</a>. </p>
<h2>Radio Buttons grouped with fieldsets</h2>
<?php includeStats(["isForNewBuilds" => true]); ?>
<p>This is the recommended way of grouping radio buttons. If you need them to be styled a different way, please look at the next few examples.</p>
...
<p> Radio buttons can be styled using a bit of careful CSS-fu. I styled these by refactoring the basic CSS from the <a href="https://codepen.io/manabox/pen/raQmpL">Custom Radio Button CSS Only Codepen</a> by <a href="http://webcreatormana.com">Mana</a>. I added focus states as well <a href="https://www.sarasoueidan.com/blog/inclusively-hiding-and-styling-checkboxes-and-radio-buttons/">ensuring that these styled facades will be discoverable to users navigating by touch</a>. </p>
...
<div id="example2" class="enable-example">
    ...
    <div role="radiogroup" aria-labelledby="alert1" class="enable-custom-radiogroup aria-form-group">
        ...
        <div>
            <span role="radio" aria-checked="false" tabindex="0" aria-labelledby="a1r1" data-name="a1radio">
            </span>
            <span id="a1r1" class="aria-radio-label">Yes</span>
        </div>
        <div>
            <span role="radio" aria-checked="false" tabindex="0" aria-labelledby="a1r2" data-name="a1radio">
            </span>
    ...
    <div role="radiogroup" aria-labelledby="alert2" class="enable-custom-radiogroup aria-form-group">
        ...
        <div>
            <span role="radio" aria-checked="false" tabindex="0" aria-labelledby="a2r1" data-name="a2radio">
            </span>
            <span id="a2r1" class="aria-radio-label">Yes</span>
        </div>
        <div>
            <span role="radio" aria-checked="false" tabindex="0" aria-labelledby="a2r2" data-name="a2radio">
            </span>
...
<p> When it comes to styling fieldsets, they can sometimes be as stubborn as my Mom when I tell her it's time for a medical checkup. Unlike my Mom, who is absolutely irreplaceable to me, I can replace fieldsets with an ARIA <code>radiogroup</code> role. </p>
...
<?php includeStats(["doNot" => false, "comment" => "<p><strong>We believe that radio buttons should look like the other examples on this page because it's what users are used to.</strong>  We only would use the code below if you had to fix code already implemented.</p><p>For net new work, do your users a favour and make radio buttons look like traditional radio buttons."]); ?>
...
<div id="circleless-radio-buttons" class="enable-example circleless-radio-buttons">
    ...
    <fieldset >
        <legend>Size</legend>
        <div class="keyboard-instructions" id="instructions1"> Use the arrow keys to choose one of the options. </div>
        <div role="group" aria-labelledby="instructions1">
            <label> <input class="sr-only" type="radio" name="size" value="100mg" /> 100mg </label>
            <label> <input class="sr-only" type="radio" name="size" value="500mg" /> 500mg </label>
            <label> <input class="sr-only" type="radio" name="size" value="1000mg" /> 1000mg </label>
        </div>
    </fieldset>
</div>
<?php includeShowcode("circleless-radio-buttons")?>
<script type="application/json" id="circleless-radio-buttons-props">
    {
      "replaceHtmlRules": {
      },
      "steps": [
      {
        "label": "Wrap labels around the radio buttons",
        "highlight": "%OPENCLOSECONTENTTAG%label",
        "notes": ""
      },
      {
        "label": "Hide the default UI",
        "highlight": "sr-only",
        "notes": "We hide it will <a href=\"screen-reader-only-text.php\">screen reader only text</a> so screen readers can read the content while being visibly hidden."
      },
      {
        "label": "Style the radio buttons to look like buttons",
        "highlight": "%CSS% radiogroup-css~ .circleless-radio-buttons fieldset ; .circleless-radio-buttons label ; .circleless-radio-buttons label:focus-within ; .circleless-radio-buttons label:has(input[type=\"radio\"]:checked) ||| outline[^;]*; ",
        "notes": "Style the labels to look like buttons.  Note that there should be a focus ring on the label if focus is applied to anything inside of it via <code>:focus-within</code>. "
      },
      {
        "label": "Style the keyboard instructions to appear when focus is applied inside the group's fieldset.",
        "highlight": "%CSS% radiogroup-css~ .circleless-radio-buttons .keyboard-instructions ||| display[^;]*; |||  %CSS% radiogroup-css~ .circleless-radio-buttons fieldset:focus-within:has(:focus-visible) .keyboard-instructions ; .enable__is-dark-mode .circleless-radio-buttons .keyboard-instructions ||| display[^;]*; ||| fieldset:focus-within:has\\(:focus-visible\\) ",
        "notes": "<code>display: none</code> is applied to the instructions by default.  <code>display: block</code> is applied when focus is applied inside the fieldset (using <code>fieldset:focus-within</code>). We ensure this happens only with keyboard users and not mouse users by adding <code>:has\\(:focus-visible\\)</code> to the end of the selector."
      }
